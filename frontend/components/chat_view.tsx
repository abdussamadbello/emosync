...
import {
  ...
  PlanGeneratedCard,
  type SuggestionData,
} from "@/components/chat-actions/suggestion-cards";

// ── Text cleaning helpers ────────────────────────────────────────────────────

/** Strip the ```suggestions JSON block (complete or still streaming). */
function strip_suggestions_block(text: string): string {
  // Complete block: ```suggestions ... ```
  let cleaned = text.replace(/```suggestions\s*\n[\s\S]*?```/g, "");
  // Partial block still streaming (no closing ```)
  cleaned = cleaned.replace(/```suggestions\s*\n[\s\S]*$/g, "");
  return cleaned.trim();
}

function parse_suggest_tag(text: string): { clean_text: string; suggest: string | null } {
  const cleaned = strip_suggestions_block(text);
  const match = cleaned.match(/\[suggest:(journal|mood_check|assessment|goal_update)\]/);
  if (!match) return { clean_text: cleaned, suggest: null };
  return {
    clean_text: cleaned.replace(match[0], "").trim(),
    suggest: match[1],
  };
}
